...
def api_upload_orders_to_shipstation():
    """Order upload to ShipStation is DISABLED. BigCommerce pushes orders directly."""
    ...
    def _process():
        try:
            ...
            try:
                ...
                logger.error(
                    f"Webhook: Step 1 (immediate processing) failed — "
                    f"falling through to full sweep: {_step1_err}",
                    exc_info=True,
                )
...
# ========================================
# PHYSICAL COUNT INVENTORY CORRECTION (Admin Only)
# ========================================

@app.route('/api/admin/physical-count-correction', methods=['POST'])
@admin_required
def admin_physical_count_correction():
    """
    Applies the Apr 17 2026 physical-count reconciliation corrections to inventory_current
    and inventory_transactions for all 5 affected SKUs.

    Corrections (all trace to Apr 10 seed restore double-counts):
      17612  Adjust Up   +298  (1,936 → 2,234)
      17904  Adjust Down   99  (  160 →    61)
      17914  Adjust Down  752  (1,445 →   693)
      18675  Adjust Down  294  (  864 →   570)
      18795  Adjust Down  172  (6,993 → 6,821)

    POST body: { "dry_run": true|false }  (defaults to true for safety)

    Idempotent: returns 409 if corrections have already been applied.
    """
    import logging
    logger = logging.getLogger(__name__)
    ...
    CORRECTIONS = [
        {
            'sku': '17612',
            'transaction_type': 'Adjust Up',
            'quantity': 298,
            'expected_before': 1936,
            'expected_after': 2234,
            'note': (
                'Physical count reconciliation 2026-04-17: seed restore tx 127 '
                'over-corrected by 298 units (lot 260047). Adjusted to match physical count.'
            ),
        },
        {
            'sku': '17904',
            ...
            'expected_after': 61,
            'note': (
                'Physical count reconciliation 2026-04-17: seed restore tx 128 '
                'inflated balance ~99 units (lot 250240). Adjusted to match physical count.'
            ),
        },
        {
            'sku': '17914',
            'transaction_type': 'Adjust Down',
            'quantity': 752,
            'expected_before': 1445,
            'expected_after': 693,
            'note': (
                'Physical count reconciliation 2026-04-17: seed restore tx 129 '
                'inflated balance ~752 units (lot 250297). Adjusted to match physical count.'
            ),
        },
        {
            'sku': '18675',
            ...
            'expected_after': 570,
            'note': (
                'Physical count reconciliation 2026-04-17: seed restore tx 130 '
                'inflated balance ~294 units (lot 240231). Adjusted to match physical count.'
            ),
        },
        {
            'sku': '18795',
            'transaction_type': 'Adjust Down',
            'quantity': 172,
            'expected_before': 6993,
            'expected_after': 6821,
            'note': (
                'Physical count reconciliation 2026-04-17: seed restore tx 131 '
                'inflated balance 172 units (lot 11001). Adjusted to match physical count.'
            ),
        },
    ]
    ...
    try:
        ...
        # Idempotency check — abort if any correction note already exists
        cursor.execute("""
            SELECT sku, quantity, transaction_type
            FROM inventory_transactions
            WHERE notes LIKE 'Physical count reconciliation 2026-04-17:%'
            ORDER BY sku;
        """)
        already_applied = cursor.fetchall()
        if already_applied:
            conn.close()
            return jsonify({
                'success': False,
                'error': 'Corrections already applied.',
                'already_applied': [
                    {'sku': r[0], 'quantity': r[1], 'type': r[2]}
                    for r in already_applied
                ],
            }), 409

        # Read current quantities for before/after report
        skus = [c['sku'] for c in CORRECTIONS]
        cursor.execute(
            "SELECT sku, current_quantity FROM inventory_current WHERE sku = ANY(%s);",
            (skus,)
        )
        current_qtys = {r[0]: r[1] for r in cursor.fetchall()}

        # Validate expected_before values match live database
        mismatches = []
        for c in CORRECTIONS:
            live = current_qtys.get(c['sku'])
            if live != c['expected_before']:
                mismatches.append({
                    'sku': c['sku'],
                    'expected': c['expected_before'],
                    'actual': live,
                })
        ...
        if mismatches:
            ...
            return jsonify({
                'success': False,
                'error': 'Live inventory values do not match expected pre-correction values. '
                         'Database may have changed since corrections were calculated.',
        ...
        if dry_run:
            ...
            return jsonify({
                'success': True,
                'dry_run': True,
                'message': 'Dry run — no changes made. POST with dry_run=false to apply.',
                'corrections': preview,
            }), 200
        ...
        for c in CORRECTIONS:
            delta = c['quantity'] if c['transaction_type'] == 'Adjust Up' else -c['quantity']

            cursor.execute("""
                INSERT INTO inventory_transactions
                    (date, sku, quantity, transaction_type, notes, created_at)
                VALUES (CURRENT_DATE, %s, %s, %s, %s, NOW());
            """, (c['sku'], c['quantity'], c['transaction_type'], c['note']))
        ...
        # Read back final quantities to confirm
        cursor.execute(
            "SELECT sku, current_quantity FROM inventory_current WHERE sku = ANY(%s);",
            (skus,)
        )
        final_qtys = {r[0]: r[1] for r in cursor.fetchall()}
        conn.close()

        results = []
        all_ok = True
        for c in CORRECTIONS:
            final = final_qtys.get(c['sku'])
            ok = final == c['expected_after']
            if not ok:
                all_ok = False
            results.append({
                'sku': c['sku'],
                'transaction_type': c['transaction_type'],
                'quantity_adjusted': c['quantity'],
                'before': c['expected_before'],
                'after': final,
        ...
        logger.info(f"Physical count correction applied: {results}")
        return jsonify({
            'success': True,
            'dry_run': False,
            'all_match': all_ok,
        ...
        logger.error(f"Physical count correction failed: {e}", exc_info=True)
        try:
