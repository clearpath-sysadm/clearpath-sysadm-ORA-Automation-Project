...
def _sop_publish_error(result):
    """Return one bounded, user-readable publisher error line."""
    output = (result.stderr or result.stdout or '').strip()
    lines = [line.strip() for line in output.splitlines() if line.strip()]
    return (lines[-1] if lines else 'The SOP publisher failed without an error message.')[:500]


@app.route('/api/sops/publish', methods=['POST'])
@admin_required
def publish_sops():
    """Regenerate workspace SOP artifacts from the fixed controlled sources."""
    if os.environ.get('REPLIT_DEPLOYMENT'):
        return jsonify({
            'success': False,
            'error': (
                'SOP regeneration is available in the workspace only. '
                'Update the source document there, regenerate it, and publish the app again.'
            ),
        }), 409

    if not _sop_publish_lock.acquire(blocking=False):
        return jsonify({
            'success': False,
            'error': 'SOP regeneration is already running.',
        }), 409

    try:
        result = subprocess.run(
            [sys.executable, os.path.join(project_root, 'scripts', 'publish_sops.py')],
            cwd=project_root,
            capture_output=True,
            text=True,
            timeout=120,
        ...
        if result.returncode != 0:
            error = _sop_publish_error(result)
            logger.warning(
                "SOP regeneration failed for admin %s: %s",
        ...
        logger.info(
            "SOP library regenerated by admin %s",
            getattr(current_user, 'email', 'unknown'),
        )
        return jsonify({
            'success': True,
            'message': message or 'SOP library regenerated successfully.',
        })
    except subprocess.TimeoutExpired:
        logger.error(
            "SOP regeneration timed out for admin %s",
            getattr(current_user, 'email', 'unknown'),
        )
        return jsonify({
            'success': False,
            'error': 'SOP regeneration timed out after 120 seconds.',
        }), 504
    except OSError:
        logger.exception(
            "Unable to start SOP regeneration for admin %s",
            getattr(current_user, 'email', 'unknown'),
        )
        return jsonify({
            'success': False,
            'error': 'The SOP publisher could not be started.',
        }), 500
    finally:
        _sop_publish_lock.release()
...
def backfill_18795():
    ...
    try:
        ...
        target = next((r for r in all_lots if r[1]=='11001'), None)
        if not target:
            conn.close()
        ...
        total_units = sum(r[2] or 0 for r in missing)
        ...
        if write:
            ...
            return f'''<!doctype html><html><head><style>
                ...
                th,td{{padding:8px 12px;border:1px solid #333;text-align:left}}th{{background:#1e1e1e}}
            </style></head><body>
        ...
        apply_btn = f'''<form method="POST" style="margin-top:24px">
            ...
            font-size:16px;border-radius:8px;cursor:pointer">▶ Apply Backfill ({len(missing)} orders, {total_units} units)</button>
        </form>''' if missing else '<p style="color:#4ade80">✅ Nothing to backfill — all orders already have Ship transactions.</p>'
        return f'''<!doctype html><html><head><style>
            body{{font-family:sans-serif;padding:32px;max-width:900px;margin:0 auto;background:#111;color:#eee}}
            h2{{color:#60a5fa}}table{{width:100%;border-collapse:collapse;margin-top:16px}}
            th,td{{padding:8px 12px;border:1px solid #333;text-align:left}}th{{background:#1e1e1e}}
            li{{margin:4px 0}}
        </style></head><body>
        <h2>18795 Backfill — Dry Run</h2>
...
def api_resync_shipped_deductions():
    ...
    try:
        ...
        # Step 1: fetch all shipped orders from ShipStation for the date range.
        # Using ShipStation as the discovery source (not orders_inbox) so that split
        # labels — which each have their own unique shipstation_order_id — are all found.
        api_key, api_secret = get_shipstation_credentials()
        ss_headers = get_shipstation_headers(api_key, api_secret)

        ss_params = {'orderStatus': 'shipped', 'pageSize': 500, 'page': 1}
        if ship_date_start:
            ss_params['shipDateStart'] = ship_date_start
        if ship_date_end:
            ss_params['shipDateEnd'] = ship_date_end
        ...
        if not all_ss_orders:
            return jsonify({'success': True, 'message': 'No shipped orders found in ShipStation for that date range',
                            'deducted': 0, 'skipped': 0, 'total_rows_found': 0})

        # Step 2: for each ShipStation label (each orderId), check inventory_transactions
        # by shipstation_order_id and deduct if missing.  deduct_lot_inventory handles
        # idempotency internally so calling it for already-deducted labels is safe.
        deducted = 0
        skipped = 0
        errors = []
        ...
        with transaction_with_retry() as conn:
            for ss_order in all_ss_orders:
                ...
                ss_order_id = str(ss_order.get('orderId', '')).strip()
                ...
                try:
                    for item in ss_order.get('items', []):
                        ...
                        if result:
                            deducted += 1
                            logger.info(f"Resync: deducted {quantity} units of {base_sku} for order {order_number} (ss_id={ss_order_id})")
                        else:
                            skipped += 1

                except Exception as _row_err:
                    errors.append(f"{order_number} (ss_id={ss_order_id}): {_row_err}")
        ...
        return jsonify({
            ...
            'skipped': skipped,
            'errors': errors,
            'total_rows_found': len(all_ss_orders),
        })
