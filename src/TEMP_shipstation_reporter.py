...
if __name__ == "__main__":
    ...
    # Fixed f-string syntax error here:
    ...
    AllDailySKUMovements_DF = pd.merge(
        ...
        DailyShippedSKUQty_DF, # Now has 'SKU' and 'Date'
        on=['Date', 'SKU'],
        how='left'
    ).fillna({'TotalQtyShipped': 0}) # Fill NaN with 0 for shipped
    AllDailySKUMovements_DF.rename(columns={'TotalQtyShipped': 'ShippedQty'}, inplace=True) # Rename back to ShippedQty for consistency
