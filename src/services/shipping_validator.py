...
# Benco ships via UPS third-party billing (ShipStation carrier code: ups, Axiom account).
    ...
    if 'BENCO' in ship_company:
        if carrier_code and carrier_code != BENCO_EXPECTED_CARRIER:
            violations.append({
                ...
                'ship_company': ship_company,
                'severity': 'CRITICAL',
                'message': f'Benco order must use UPS Ground (third-party billing, carrier=ups), currently using {carrier_code or "unknown carrier"}'
            })
