...
- **Backend Automation (Python scripts):**
    - **Unified ShipStation Sync (`src/unified_shipstation_sync.py`):** Production workflow that combines status sync and manual order import into single watermark-based process running every 5 minutes. Features comprehensive emoji-based logging, transaction-safe watermarking with 14-day lookback window, and only advances watermark on successful processing (zero errors). Replaces deprecated `manual_shipstation_sync.py` and `shipstation_status_sync.py`. **NEW (Oct 23, 2025):** Integrated real-time tracking status polling using ShipStation's `/shipments` API endpoint. Displays tracking status icons (✅📦🚚⚠️❓) next to tracking numbers in Orders Inbox UI. Features business hours polling (6 AM Pacific to 5 PM Eastern, 11-hour window), 5-minute cadence for undelivered orders, automatic exception alerting, and retry storm protection (timestamps updated on all code paths). Service uses ShipStation Basic-auth with existing credentials (no new API keys required). See `/docs/features/TRACKING_STATUS_IMPLEMENTATION_LOG.md` for complete implementation details.
