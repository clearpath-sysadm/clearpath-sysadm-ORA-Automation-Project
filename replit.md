...
- **Backend Automation (Python scripts):**
    ...
    - **Duplicate Order Monitoring System:** Scans ShipStation every 15 minutes for duplicate order numbers with intelligent auto-resolution and two-tier resolution options. Features **"Mark Resolved"** (temporary dismissal) and **"Permanently Exclude"** (forever excludes order+SKU from detection) buttons with admin-only access. The `excluded_duplicate_orders` table stores permanent exclusions; the scanner checks this table before creating alerts. Auto-resolution evaluates remaining non-deleted records: alerts resolve when (1) duplicates no longer appear in scans, OR (2) all duplicate records are deleted, OR (3) after user deletions, remaining records no longer constitute duplicates/mismatches. Dashboard features asynchronous modal updates—when a duplicate record is deleted and the remaining records are clean, the alert automatically disappears from the modal without page refresh. Manual resolution and order deletion operations update the UI in real-time with smooth animations. The modal displays deleted orders with red "🗑️ DELETED" badges and disabled actions, automatically refreshing on error to show current state. Also detects manual order conflicts where the same order number has different ShipStation IDs.
