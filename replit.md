...
- **Shipping Validation System (IN PROGRESS):** Database schema extended with carrier/service tracking fields (`shipping_carrier_code`, `shipping_carrier_id`, `shipping_service_code`, `shipping_service_name`). Upload workflow automatically assigns FedEx 2Day for Hawaiian orders and FedEx International Ground for Canadian orders. Benco carrier account validation requires post-upload carrier_id capture (pending implementation in status sync). Violations table created for alert system.
