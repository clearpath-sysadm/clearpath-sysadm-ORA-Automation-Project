...
- **Shipping Validation System (IN PROGRESS):** ALERT-ONLY system (does not modify ShipStation orders). Database tracks carrier/service info in 4 fields (`shipping_carrier_code`, `shipping_carrier_id`, `shipping_service_code`, `shipping_service_name`). Status sync and manual order sync capture actual carrier/service from ShipStation. Validation service (`src/services/shipping_validator.py`) compares actual vs. expected for 3 rules: (1) Hawaiian→FedEx 2Day, (2) Canadian→International Ground, (3) Benco→Benco FedEx account. Violations stored in `shipping_violations` table with resolved status tracking. API endpoints and sticky alert banner UI pending implementation.
