...
- **Deployment:** Configured as a VM deployment in Replit using `start_all.sh` startup script that launches all automation workflows in parallel. Runs continuously on Reserved VM ($20/month, covered by Core plan credits). All processes start automatically on deployment: dashboard server (port 5000), XML polling (5 min), status sync (hourly), manual order sync (hourly), cleanup (daily), units refresh, and weekly reporter. Single deployment runs all 6 background automation workflows plus dashboard.
