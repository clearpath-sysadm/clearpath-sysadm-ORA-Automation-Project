...
- **Workflow Health Monitoring (Jan 17, 2026):** Heartbeat logging for all workflows (`workflow_heartbeats` table). UI enhancements on Workflow Controls page (health badges, reset buttons). APIs: `/api/workflow_health`, `/api/workflow/{name}/reset`. Note: the original stuck-workflow-detector scheduled script was later removed as redundant; the heartbeat health API and dashboard UI remain active.
