...
- **Backend Automation (Python scripts):**
    ...
    - **Duplicate Order Monitoring System:** Scans ShipStation every 15 minutes for duplicate order numbers with intelligent auto-resolution (alerts resolve when duplicates no longer appear in scans OR when all duplicate records are deleted). Dashboard features asynchronous modal updates—when a duplicate record is deleted and only one remains, the alert automatically disappears from the modal without page refresh. Manual resolution and order deletion operations update the UI in real-time with smooth animations. Also detects manual order conflicts where the same order number has different ShipStation IDs.
