...
- **Deployment:** Configured as a VM deployment in Replit, serving the dashboard on port 5000, with scheduled workflows: `dashboard-server`, `xml-import-scheduler`, `shipstation-status-sync`, `shipstation-units-refresh`, `weekly-reporter`, and `orders-cleanup`.
