...
- **Backend Automation (Python scripts):**
    ...
    - **Environment Protection System:** Upload service (`src/scheduled_shipstation_upload.py`) automatically detects dev environment via REPL_SLUG and enters sleep mode to prevent dev-created duplicate orders in production ShipStation. Production deployments work normally with same codebase.
