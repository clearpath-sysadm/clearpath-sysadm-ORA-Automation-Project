...
def test_help_training_is_visible_and_requires_authentication():
    styles = (ROOT / "static/css/global-styles.css").read_text()
    app = (ROOT / "app.py").read_text()
    match = re.search(r"^ALLOWED_PAGES\s*=\s*(\[.*\])$", app, re.MULTILINE)

    assert '.sidebar-nav a[href="/help.html"]' not in styles
    assert match
    assert "help.html" in ast.literal_eval(match.group(1))
    assert (ROOT / "help.html").exists()
    assert "filename != 'landing.html' and not current_user.is_authenticated" in app
